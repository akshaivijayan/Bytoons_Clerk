...
loadBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    fieldCountEl.textContent = data.count || allFields.length;
    renderFields(allFields);
    setStatus('Loaded');
  } catch (err) {
    setStatus('Error');
    setMessage('Failed to load fields', 'error');
  }
});
...
fillBtn.addEventListener('click', async () => {
  ...
  try {
    const res = await fetch('/api/fill', {
      ...
      body: JSON.stringify({ values: payload, flatten: flattenToggle.checked }),
    });
